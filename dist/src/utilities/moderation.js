    ...
    for (const banCase of expiredBans) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor("#3498DB")
                .setTimestamp()
                .addFields({ name: "👤 User", value: `\`${targetUser.tag}\` (\`${banCase.userId}\`)` });
            const channel = client.channels.cache.get(channels.moderationLogs);
...
export async function getActiveCasesByUser(guildId, userId) {
...
export async function getActiveCaseByUserAndInfraction(guildId, userId, action) {
...
export async function getCasesByInfraction(guildId, action) {
    return CaseModel.find({ guildId, action });
}
export async function registerCase({ action, guildId, userId, issuerId, reason, duration, client }) {
    ...
    if (!targetUser || !issuerUser || !guild)
        return;
    ...
    const isTimedPunishment = action === Infraction.BAN || action === Infraction.MUTE;
    // Direct Messages
    const dmEmbed = new EmbedBuilder()
        .setDescription(`You have been ${pastTense} ${locationWord} **${guild.name}** for reason: ${reason}${isTimedPunishment
        ? duration && expiry
